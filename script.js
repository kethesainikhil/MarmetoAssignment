...
const createProductCard = (product) => {
    ...
    console.log(product.badge_text)
    if((product.badge_text == null)) {
        
        console.log("hey one iem")
        badgeText.style.display = 'none';
    }
    else{
        console.log("hey two iem")
    ...
    if (product.title.length > 10) {
        product.title = product.title.substring(0, 10); // Get the first 10 characters
    ...
    // compareAtPrice.style.textDecoration = 'line-through'; // Apply line-through to compareAtPrice

    // Calculate discount percentage
    const discount = Math.round(((product.compare_at_price - product.price) / product.compare_at_price) * 100);
    const discountText = document.createElement('span'); // Use span instead of div
    ...
    if (discount > 0) {
        ...
        discountText.style.textDecoration = 'none'; // Remove text decoration from discountText
    } else {
        discountText.style.textDecoration = 'none'; // Ensure text decoration is removed even if discount is 0
    }
    ...
    // Append discount text to compareAtPrice
    
    details.appendChild(lastColumn)// Append compareAtPrice to details
}
...
tabs.forEach(tab => {
    tab.addEventListener('click', (e) => {
        ...
        fetchAndDisplayProducts(category)
            ...
            otherTab.style.backgroundColor = ''; // Reset background color
            const otherImg = otherTab.querySelector('.icon');
            otherImg.style.opacity = ''; // Reset opacity
        });
            ...
            // Set the opacity of the image to 100%
            img.style.opacity = '1';
